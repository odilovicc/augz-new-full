<template>
  <div class="p-8">
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Вопросы</h1>
        <p class="text-sm text-gray-400 mt-0.5">{{ total }} вопросов</p>
      </div>
      <!-- Status filter tabs -->
      ...
      <table v-else class="w-full text-sm">
        <thead class="border-b border-gray-100 dark:border-gray-800">
          <tr class="text-left text-gray-400 text-xs uppercase tracking-wide">
            <th class="px-5 py-3 font-medium">Отправитель</th>
            <th class="px-5 py-3 font-medium">Тема</th>
            <th class="px-5 py-3 font-medium">Вопрос</th>
        ...
        <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
          <tr v-for="item in filtered" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
            ...
            <td class="px-5 py-3">
              <UBadge variant="soft" color="neutral" size="xs">{{ item.category }}</UBadge>
            </td>
            <td class="px-5 py-3 max-w-xs">
              <p class="text-gray-600 dark:text-gray-300 text-xs line-clamp-2">{{ item.question }}</p>
            </td>
            <td class="px-5 py-3">
              <span
          ...
          <tr v-if="!filtered.length">
            <td colspan="6" class="px-5 py-12 text-center text-gray-400">Нет вопросов</td>
          </tr>
        </tbody>
      </table>
    </div>
  ...
  <!-- Slideover -->
  <USlideover v-model:open="slideOpen" title="Вопрос" side="right" class="w-full max-w-lg">
    <template #body>
      <div v-if="editItem" class="flex flex-col gap-5 p-6">
        <div class="flex flex-col gap-3 p-4 rounded-xl bg-gray-50 dark:bg-gray-800">
          <div class="grid grid-cols-2 gap-3 text-sm">
            ...
            <div>
              <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-0.5">Тема</p>
              <p class="text-gray-700 dark:text-gray-200">{{ editItem.category }}</p>
            </div>
            <div>
          ...
          <div>
            <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1">Вопрос</p>
            <p class="text-sm text-gray-700 dark:text-gray-200 leading-relaxed whitespace-pre-wrap">{{ editItem.question }}</p>
          </div>
          <div>
            <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-0.5">Дата</p>
            <p class="text-sm text-gray-500">{{ formatDate(editItem.created_at) }}</p>
          </div>
        </div>
...
const statuses = [
  ...
  { value: 'in_progress', label: 'В работе',  dot: 'bg-amber-500',  activeBg: 'bg-amber-50 dark:bg-amber-950', activeText: 'text-amber-700 dark:text-amber-300' },
  { value: 'answered',    label: 'Отвечено',  dot: 'bg-green-500',  activeBg: 'bg-green-50 dark:bg-green-950', activeText: 'text-green-700 dark:text-green-300' },
]
...
const badgeMap: Record<string, string> = {
  ...
  answered:    'bg-green-50 text-green-700 dark:bg-green-950 dark:text-green-300',
}
...
const { data, pending, refresh } = await useAsyncData(
  'admin-questions',
  () => apiFetch<{ data: any[] }>('/admin/questions'),
)
...
async function save() {
  ...
  saveError.value = ''
  try {
    await apiFetch(`/admin/questions/${editItem.value.id}`, {
      method: 'PUT',
