<template>
  <div class="p-8">
    <div class="flex items-center justify-between mb-6">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Заявки на членство</h1>
      ...
      <table v-else class="w-full text-sm">
        <thead class="border-b border-gray-100 dark:border-gray-800">
          <tr class="text-left text-gray-400 text-xs uppercase tracking-wide">
            ...
            <th class="px-5 py-3 font-medium">Контакты</th>
        ...
        <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
          <tr v-for="item in filtered" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
            <td class="px-5 py-3">
              ...
              <p class="text-xs text-gray-400 mt-0.5">{{ item.organization }}</p>
            </td>
            <td class="px-5 py-3">
              <p class="text-gray-600 dark:text-gray-300 text-xs">{{ item.phone }}</p>
              <p class="text-gray-400 text-xs">{{ item.email }}</p>
            </td>
            <td class="px-5 py-3">
              <UBadge variant="soft" color="neutral" size="xs">{{ planLabel(item.plan_slug) }}</UBadge>
            </td>
            <td class="px-5 py-3">
              <span
          ...
          <tr v-if="!filtered.length">
            <td colspan="6" class="px-5 py-12 text-center text-gray-400">Нет заявок</td>
          </tr>
        </tbody>
      </table>
    </div>
  ...
  <!-- Slideover -->
  <USlideover v-model:open="slideOpen" title="Заявка на членство" side="right" class="w-full max-w-lg">
    <template #body>
      <div v-if="editItem" class="flex flex-col gap-5 p-6">

        <!-- Applicant info (read-only) -->
        <div class="flex flex-col gap-3 p-4 rounded-xl bg-gray-50 dark:bg-gray-800">
          <div class="grid grid-cols-2 gap-3 text-sm">
            <div>
              <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-0.5">Имя</p>
              <p class="font-medium text-gray-900 dark:text-white">{{ editItem.name }}</p>
            </div>
            <div>
              <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-0.5">Организация</p>
              <p class="text-gray-700 dark:text-gray-200">{{ editItem.organization || '—' }}</p>
            </div>
            <div>
            ...
            <div>
              <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-0.5">Тариф</p>
              <p class="text-gray-700 dark:text-gray-200">{{ planLabel(editItem.plan_slug) }}</p>
            </div>
            <div>
              <p class="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-0.5">Дата</p>
              <p class="text-gray-700 dark:text-gray-200">{{ formatDate(editItem.created_at) }}</p>
            </div>
          </div>
        </div>
        ...
        <!-- Admin note -->
        <UFormField label="Заметка администратора">
          <UTextarea v-model="form.admin_note" :rows="3" class="w-full" placeholder="Внутренняя заметка..." />
        </UFormField>
...
const statuses = [
  ...
  { value: 'accepted', label: 'Принято',  dot: 'bg-amber-500',  activeBg: 'bg-amber-50 dark:bg-amber-950', activeText: 'text-amber-700 dark:text-amber-300' },
  { value: 'replied',  label: 'Отвечено', dot: 'bg-purple-500', activeBg: 'bg-purple-50 dark:bg-purple-950', activeText: 'text-purple-700 dark:text-purple-300' },
  { value: 'resolved', label: 'Решено',   dot: 'bg-green-500',  activeBg: 'bg-green-50 dark:bg-green-950', activeText: 'text-green-700 dark:text-green-300' },
]
...
function planLabel(slug: string | null) {
  return slug ? (planLabels[slug] ?? slug) : '—'
}

function statusStyle(status: string) {
  return statuses.find(s => s.value === status) ?? { label: status, badge: 'bg-gray-100 text-gray-600', dot: 'bg-gray-400' }
}

// Badge classes for inline display
...
interface FormState { status: string; admin_note: string }
const form = reactive<FormState>({ status: 'new', admin_note: '' })

const { data, pending, refresh } = await useAsyncData(
  'admin-applications',
  () => apiFetch<{ data: any[] }>('/admin/membership-applications'),
)
...
async function save() {
  ...
  saveError.value = ''
  try {
    await apiFetch(`/admin/membership-applications/${editItem.value.id}`, {
      method: 'PUT',
      body: { status: form.status, admin_note: form.admin_note },
    })
...
async function deleteItem(id: number) {
  deleting.value = id
  try {
    await apiFetch(`/admin/membership-applications/${id}`, { method: 'DELETE' })
    await refresh()
  } finally {
    deleting.value = null
  }
